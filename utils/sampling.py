...
def cifar_noniid(dataset, num_users):
    ...
    for i in idx_shard:
        rest_idxs = np.concatenate((rest_idxs, idxs[i*num_imgs:(i+1)*num_imgs]), axis=0)
    num_items = int(len(dataset)/num_users)
    for i in range(int(num_users * 0.8), num_users):
        dict_users[i] = set(np.random.choice(rest_idxs, num_items, replace=False))
        rest_idxs = list(set(rest_idxs) - dict_users[i])
